...
/*fixme: quick hack for stubs at end of beam and different stem directions*/
	...
	for (i = 2; i <= bm->nflags; i++) {
		...
		for (s = s1; ; s = s->next) {
			...
			if (s->as.type != ABC_T_NOTE || s->nflags < i) {
				if (s == s2)
					break;
				continue;
			}
...
/* -- draw a measure bar -- */
static void draw_bar(struct SYMBOL *s, float h)
{
...
static float set_staff(void)
{
	...
	for (staff = 0; staff <= nstaff; staff++)
		delta_tb[staff].not_empty = !sy->staff[staff].empty;
